...
class Wave11SiteTests(unittest.TestCase):
    ...
    def test_results_use_relationship_rows_not_a_repeated_card_grid(self) -> None:
        self.assertIn('class="case-row"', self.app)
        self.assertNotIn("case-card", self.html)
        self.assertNotIn("case-card", self.app)
        self.assertNotIn("case-card", self.css)
        results_rule = self.css.split(".results {", 1)[1].split("}", 1)[0]
        self.assertNotIn("display: grid", results_rule)
        self.assertNotIn("repeat(3", results_rule)
    ...
    def test_readme_preserves_context_commands_and_publication_boundary(self) -> None:
        ...
        self.assertIn("Wave 5 acceptance runs locally", self.readme)
        self.assertIn("Public publication is authorized only after Wave 5 completes", self.readme)
